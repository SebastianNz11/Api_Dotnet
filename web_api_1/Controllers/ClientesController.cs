...
namespace web_api_1.Controllers{
    ...
    public class ClientesController : Controller{
        ...
        public async Task<ActionResult> Put([FromBody] Clientes clientes){
            var v_cliente = dbConexion.Clientes.SingleOrDefault(a => a.id_cliente == clientes.id_cliente);
            if (v_cliente != null && ModelState.IsValid){
               dbConexion.Entry(v_cliente).CurrentValues.SetValues(clientes); 
               await dbConexion.SaveChangesAsync();
               return Ok();
            }else
            {
